...
data class RegistreRetrait(
    override val cle: String,
    ...
    fun toDcObject(baseUri: String, msCle: String, clePersonne: String, pieceUri: String): DCBusinessResourceLight {
        ...
        if (!siret.isEmpty()) resourceLight.setStringValue(
            "mpretrait:entreprise",
            entrepriseUri
        ) //siret not always present in Marchés Sécurisés Reponse or retraits
    ...
    companion object {
        ...
        fun fromSoapObject(
            ...
            return RegistreRetrait(
                cle = responseObject.properties!![0].value!!,
                ...
                dateDebut = LocalDateTime.ofInstant(
                    Instant.ofEpochSecond((responseObject.properties[7].value!!).toLong()),
                    TimeZone.getDefault().toZoneId()
                ),
                dateFin = LocalDateTime.ofInstant(
                    Instant.ofEpochSecond((responseObject.properties[9].value!!).toLong()),
                    TimeZone.getDefault().toZoneId()
                ),
                personne = personne,
